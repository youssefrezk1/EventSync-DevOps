...
const AgendaBuilder: React.FC<AgendaBuilderProps> = ({
  ...
  const handleSaveSession = () => {
    ...
    if (formData.date < workshopStartDate) {
      alert(`Session date cannot be before ${formatDate(workshopStartDate)}`);
      return;
    }
    if (formData.date > workshopEndDate) {
      alert(`Session date cannot be after ${formatDate(workshopEndDate)}`);
      return;
    }
    if (formData.date === workshopStartDate && formData.startTime < workshopStartTime) {
      alert(
        `Session on ${formatDate(workshopStartDate)} cannot start before ${formatTime(workshopStartTime)}`
      );
      return;
    }
    if (formData.date === workshopEndDate && formData.endTime > workshopEndTime) {
      alert(
        `Session on ${formatDate(workshopEndDate)} cannot end after ${formatTime(workshopEndTime)}`
      );
      return;
    }
    ...
    if (overlap) {
      alert(
        `This session overlaps with "${overlap.title || "Break Time"}" (${formatTime(
          overlap.startTime
        )} - ${formatTime(overlap.endTime)}).`
      );
      return;
    }
            ...
            <div className="p-3 space-y-3">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Workshop dates: {formatDate(workshopStartDate)} - {formatDate(workshopEndDate)}
                </p>
              </div>
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {["Dr.", "Prof.", "Assoc. Prof."].map((title) => {
                        const parts = formData.professor.split(" ").filter(Boolean);
                        const knownTitles = ["Dr.", "Prof.", "Assoc. Prof."];
                        
                        let currentTitle;
                        if (parts[0] === "Assoc." && parts[1] === "Prof.") {
                          currentTitle = "Assoc. Prof.";
                        } else if (knownTitles.includes(parts[0])) {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              ...
                              let withoutTitle;
                              if (parts[0] === "Assoc." && parts[1] === "Prof.") {
                                withoutTitle = parts.slice(2);
                              } else if (knownTitles.includes(parts[0])) {
                                withoutTitle = parts.slice(1);
                              } else {
                                withoutTitle = parts;
                              }
                              
                              const name = withoutTitle.join(" ");
                              
                              setFormData({
                                ...formData,
                                professor: `${title}${name ? " " + name : ""}`.trim(),
                              });
                            }}
                            className={`px-2 py-1.5 text-xs font-medium border transition ${
                              currentTitle === title
                                ? "bg-[#003d52] text-white border-[#003d52]"
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setFormData({
                            ...formData,
                            professor: `${title} ${e.target.value}`.trim(),
                          });
                        }}
                        placeholder="Full Name"
                        className="flex-1 px-2 py-1.5 text-xs border border-gray-300 focus:outline-none focus:ring-1 focus:ring-[#003d52] focus:border-transparent"
                      />
                    </div>
                  </div>
