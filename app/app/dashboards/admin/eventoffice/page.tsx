...
export default function AdminsPage() {
  ...
  const fetchEventOffices = async () => {
    try {
      console.log("Fetching event offices...");
      setLoading(true);
      const res = await api.get("/admin/getEventOffices");
      console.log("Fetched event offices:", res.data.eventOffices);
      const fetchedAdmins = res.data.eventOffices || [];
      setAllAdmins(fetchedAdmins);
      setAdmins(fetchedAdmins);
    } catch (err) {
      console.error("Error fetching event offices:", err);
      setPageError("Failed to fetch event offices");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.log("Sending create EventOffice request...");

      const response = await api.post("/admin/createEventOffices", payload);

      console.log("Create EventOffice response:", response);

      if (response.status >= 200 && response.status < 300) {
        console.log("EventOffice created successfully, refreshing list...");
        await fetchEventOffices();
        console.log("EventOffice list refreshed, closing modal...");
        handleCloseModal();
        setSuccess("EventOffice created successfully!");
        setTimeout(() => setSuccess(null), 3000);
      } else {
        throw new Error(response.data?.message || "Failed to create event office");
      }
    } catch (err: any) {
      console.error("Error creating event office:", err);
      setFormError(
        err.response?.data?.message || err.message || "Failed to create event office"
      );
    } finally {
  ...
  const handleOpenDeleteDialog = (id: string, eventOfficeName: string) => {
    setDeleteDialog({
      open: true,
      adminId: id,
      adminName: eventOfficeName,
    });
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await api.delete(`/admin/deleteEventOffices/${adminId}`);
      await fetchEventOffices();
      setSuccess(`Event Office "${adminName}" deleted successfully!`);
      setTimeout(() => setSuccess(null), 3000);
    } catch (err: any) {
      console.error(err);
      setPageError(err.response?.data?.message || "Failed to delete event office");
    } finally {
        ...
        <Box sx={{ minHeight: "100vh" }}>
          ...
          <Box
            sx={{
              position: "relative",
              background:
                "linear-gradient(to bottom, rgba(0, 20, 40, 0.95), rgba(0, 61, 82, 0.85)), url('https://images.unsplash.com/photo-1486312338219-ce68d2c6f44d?w=1600')",
          ...
          >
            <Box sx={{ position: "relative", zIndex: 1, px: 4, py: 6, width: "100%" }}>
              ...
              >
                Manage event office staff. Create and remove event office accounts.
              </Typography>
          ...
          <Box sx={{ px: 4, mb: 3 }}>
            ...
            >
              ...
              <Grid container spacing={2} alignItems="center">
                ...
                <Grid size={{xs: 12, sm: 8, md: 9}}>                  <TextField
                    fullWidth
          ...
          <Box sx={{ px: 4, pb: 4 }}>
            ...
            <Box sx={{ mx: -4 }}>
              <DataTable
                ...
                data={admins}
                loading={false}
                emptyMessage='No event office accounts found. Click the "Add Event Office" button to create one.'
          ...
          <Dialog open={showModal} onClose={handleCloseModal} maxWidth="xs" fullWidth>
            ...
            <DialogContent dividers>
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2, pt: 1 }}>
                <TextField
                  fullWidth
                  label="Event Office Name "
                  value={formData.adminName}
                  onChange={(e) => setFormData({ ...formData, adminName: e.target.value })}
                  placeholder="Enter event office name"
            ...
            <DialogActions sx={{ p: 2 }}>
              ...
              >
                {submitting ? "Creating..." : "Create Event Office"}
              </Button>
            </DialogActions>
          </Dialog>
    ...
    >
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          <Typography variant="body2" sx={{ color: "#475569" }}>
            <strong>Role:</strong> Event Office Staff
          </Typography>
        </Box>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <Trash2 size={18} color="#64748b" />
          <Typography variant="body2" sx={{ color: "#475569" }}>
            <strong>Action:</strong> Delete Event Office
          </Typography>
        </Box>
      </Box>
      <Box
      ...
      >
        <Typography variant="caption" sx={{ color: "#64748b", fontStyle: "italic" }}>
          This event office account will lose all access to the system.
        </Typography>
      </Box>
    </Box>
  </DialogContent>
