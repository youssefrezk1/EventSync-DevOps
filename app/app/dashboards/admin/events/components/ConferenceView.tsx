...
import { useEvents, SearchFilters, useUser } from "../../../../shared/services";
...
export default function ConferenceView() {
  ...
  const fetchProfessors = async () => {
    try {
      ...
      const response = await api.get("/api/conferences/professor"); 
  ...
  const { events, loading, error, fetchEvents, deleteConference } = useEvents("conferences");
  
  // Delete dialog state
  const [selectedConferenceToDelete, setSelectedConferenceToDelete] = useState<any | null>(null);
  ...
  const handleOpenRegister = (conference: any) => {
  ...
  return (
    <Box sx={{ display: "flex", gap: 3 }}>
                  ...
                  >
                    ...
                    >
                      ...
                      {conference.ProfCreator && (
                        ...
                        >
                          ...
                          >
                            {conference.ProfCreator.firstName?.charAt(0)}
                            {conference.ProfCreator.lastName?.charAt(0)}
                          </Avatar>
                          <Typography
                            variant="caption"
                            color="text.secondary"
                          >
                            {conference.ProfCreator.firstName}{" "}
                            {conference.ProfCreator.lastName}
                          </Typography>
                        </Box>
                      )}
                    </CardContent>
                    ...
                    >
                      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                        {/* View Details Button */}
                        <Link href={`/confrence/${conference._id}`} style={{ textDecoration: 'none', flex: 1 }}>
                        <Button
                          variant="outlined"
                          fullWidth
                        ...
                        <Button
                          ...
                          color="error"
                          onClick={() => {
                            setSelectedConferenceToDelete(conference);
                            setDeleteError('');
      ...
      >
          ...
          >
            <Box flex={1}>
              <Stack direction="row" spacing={1} mb={1.5}>
                ...
                />
                <Chip
                  label={selectedConference?.sourceOfFunding}
                  variant="outlined"
                  size="small"
                  sx={{ fontWeight: 600, fontSize: "0.75rem" }}
                />
        ...
        <DialogContent sx={{ p: 3 }}>
          <Stack spacing={2.5}>
              ...
              <Box>
                ...
                <Stack spacing={1.5} sx={{ mt: 1 }}>
                  <Box display="flex" alignItems="center">
                    ...
                    >
                      {selectedConference.ProfCreator.firstName?.charAt(0)}
                      {selectedConference.ProfCreator.lastName?.charAt(0)}
                    </Avatar>
                    <Box>
            ...
            {selectedConference?.ProfCreator && <Divider />}
                ...
                <Box display="flex" alignItems="top">
                  ...
                  <Box display="flex" gap={1}>
                    ...
                    <Typography variant="body2" fontWeight={500}>
                      {formatDateTime(selectedConference?.start || "")} -{" "}
                      {formatDateTime(
                        selectedConference?.endDate || selectedConference?.start || ""
                      )}
                    </Typography>
                  </Box>
                </Box>

                {selectedConference?.registrationDeadline && (
                  <Box display="flex" alignItems="top">
                    <EventBusy
                    ...
                    <Box display="flex" gap={1}>
                      ...
                      <Typography
                        ...
                        color="error"
                      >
                        {formatDateTime(selectedConference.registrationDeadline)}
                      </Typography>
                    </Box>
                  </Box>
                )}
                ...
                {selectedConference?.capacity && (
                  <Box display="flex" alignItems="top">
                    ...
                    <Box display="flex" gap={1}>
                      ...
                      <Typography variant="body2" fontWeight={500}>
                        {selectedConference?.capacity} participants
                      </Typography>
                    </Box>
                  </Box>
                )}
            ...
            <Box>
              ...
              >
                {selectedConference?.shortDescription ||
            ...
            <Box sx={{ mt: 2 }}>
              <EventComments
                ...
                eventId={selectedConference?._id}
                eventName={selectedConference?.name}
                userRole={user?.role}
                userId={user?.id}
              />
            </Box>
        ...
        <DialogActions sx={{ p: 2.5 }}>
          ...
          <Button
            onClick={() => {
              handleCloseDetails();
              handleOpenRegister(selectedConference);
            }}
            variant="contained"
            sx={{
              borderRadius: 2,
      ...
      <Dialog
        ...
        onClose={() => { setDeleteDialogOpen(false); setSelectedConferenceToDelete(null); setDeleteError(''); }}
      >
        <DialogTitle>Delete Conference</DialogTitle>
        <DialogContent>
          <Typography>
            {selectedConferenceToDelete
              ? `Are you sure you want to delete the conference "${selectedConferenceToDelete.name || 'Unknown'}"?`
        ...
        <DialogActions>
          <Button onClick={() => { setDeleteDialogOpen(false); setSelectedConferenceToDelete(null); setDeleteError(''); }} disabled={deleteLoading}>Cancel</Button>
          <Button
  onClick={async () => {
    ...
    try {
      ...
      // Call your new delete endpoint
      const response = await api.delete(`/api/conferences/${selectedConferenceToDelete._id}`);
      ...
      if (response.status === 200) {
        ...
        setDeleteError('Failed to delete conference');
      }
    } catch (err: any) {
      setDeleteError(err.response?.data?.message || err.message || 'Failed to delete conference');
    } finally {
...
>
  {deleteLoading ? <CircularProgress size={18} color="inherit" /> : 'Delete'}
      ...
      {selectedConference && (
        ...
        >
          <DialogTitle>
            Register for {selectedConference?.name}
            <IconButton
