...
export default function BazaarView() {
  ...
  const fetchProfessors = async () => {
    try {
      ...
      const response = await api.get("/api/bazaars/professor"); 
  ...
  const { events, loading, error, fetchEvents, deleteBazaar } = useEvents("bazaars");
  
  // Delete dialog state
  const [selectedBazaarToDelete, setSelectedBazaarToDelete] = useState<any | null>(null);
                      ...
                      >
                        <Box sx={{ flex: 1, minWidth: 0, mr: 2 }}>
                          <Stack direction="row" spacing={0.5} mb={0.5}>
                            ...
                            />
                            {bazaar.facultyResponsible && (
                              <Chip
                                label={bazaar.facultyResponsible}
                                size="small"
                                sx={{
                                  backgroundColor: `${eventTypeColor}12`,
                                  color: "black",
                        ...
                        {bazaar.registrationDeadline && (
                          ...
                          >
                            ...
                            <Typography
                              ...
                              fontWeight={600}
                            >
                              Deadline: {formatDate(bazaar.registrationDeadline)}
                            </Typography>
                          </Box>
                        )}
                      </Box>
                    ...
                    >
                      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                        ...
                        <Button
                          variant="outlined"
                          fullWidth
                          //onClick={() => handleViewDetails(bazaar)}
                          sx={{
                        ...
                        <Button
                          ...
                          color="error"
                          onClick={() => {
                            setSelectedBazaarToDelete(bazaar);
                            setDeleteError('');
      ...
      >
          ...
          >
            <Box flex={1}>
              <Stack direction="row" spacing={1} mb={1.5}>
                ...
                />
                {selectedBazaar?.facultyResponsible && (
                  <Chip
                    label={selectedBazaar.facultyResponsible}
                    variant="outlined"
                    size="small"
                    sx={{ fontWeight: 600, fontSize: "0.75rem" }}
                  />
        ...
        <DialogContent sx={{ p: 3 }}>
          <Stack spacing={2.5}>
              ...
              <Box>
                ...
                <Stack spacing={1.5} sx={{ mt: 1 }}>
                  <Box display="flex" alignItems="center">
                    ...
                    >
                      {selectedBazaar.ProfCreator.firstName?.charAt(0)}
                      {selectedBazaar.ProfCreator.lastName?.charAt(0)}
                    </Avatar>
                    <Box>
                      <Typography variant="subtitle1" fontWeight={600}>
                        {selectedBazaar.ProfCreator.firstName} {selectedBazaar.ProfCreator.lastName}
                      </Typography>
            ...
            {selectedBazaar?.ProfCreator && <Divider />}
            ...
            <Box>
              ...
              <Stack spacing={1.5} sx={{ mt: 1 }}>
                ...
                {selectedBazaar?.registrationDeadline && (
                  <Box display="flex" alignItems="top">
                    <EventBusy
            ...
            <Box sx={{ mt: 2 }}>
              <EventComments
                eventType="bazaar"
                eventId={selectedBazaar?._id}
                eventName={selectedBazaar?.name}
                userRole={user?.role}
                userId={user?.id}
              />
            </Box>
  ...
  onClose={() => { 
    ...
    setSelectedBazaarToDelete(null); 
    setDeleteError(''); 
  }}
>
  <DialogTitle>Delete Bazaar</DialogTitle>
  <DialogContent>
    <Typography>
      {selectedBazaarToDelete
        ? `Are you sure you want to delete the bazaar "${selectedBazaarToDelete.name || 'Unknown'}"?`
        : 'Are you sure you want to delete this bazaar?'}
    </Typography>
  ...
  <DialogActions>
    ...
    <Button
      onClick={async () => {
        if (!selectedBazaarToDelete) return;
        try {
          setDeleteLoading(true);
          setDeleteError('');
          
          // Call the delete endpoint
          const response = await api.delete(`/bazaarbyid/${selectedBazaarToDelete._id}`);
          ...
          if (err.response) {
            switch (err.response.status) {
              case 404:
                setDeleteError('Bazaar not found. It may have already been deleted.');
                break;
              case 400:
                setDeleteError('Cannot delete this bazaar. There are vendors registered to it.');
                break;
              case 500:
                setDeleteError('Server error occurred. Please try again later.');
                break;
              default:
                setDeleteError(err.response.data?.error || err.response.data?.message || 'Failed to delete bazaar');
            }
          } else if (err.request) {
