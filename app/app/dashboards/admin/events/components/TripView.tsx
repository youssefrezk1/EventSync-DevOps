...
export default function TripView() {
  ...
  const fetchProfessors = async () => {
    try {
      ...
      const response = await api.get("/api/trips/professor"); 
  ...
  const { events, loading, error, fetchEvents, deleteTrip } = useEvents("trips");
  
  // Delete dialog state
  const [selectedTripToDelete, setSelectedTripToDelete] = useState<any | null>(null);
  ...
  return (
    <Box sx={{ display: "flex", gap: 3 }}>
                  ...
                  >
                      ...
                      >
                        <Box sx={{ flex: 1, minWidth: 0, mr: 2 }}>
                          <Stack direction="row" spacing={0.5} mb={0.5}>
                            ...
                            />
                            {trip.facultyResponsible && (
                              <Chip
                                label={trip.facultyResponsible}
                                size="small"
                                sx={{
                                  backgroundColor: `${eventTypeColor}12`,
                                  color: "black",
                    ...
                    >
                      <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                        {/* View Details Button */}
                        <Link href={`/trip/${trip._id}`} style={{ textDecoration: 'none', flex: 1 }}>
                        <Button
                          variant="outlined"
                          fullWidth
      ...
      >
          ...
          >
            <Box flex={1}>
              <Stack direction="row" spacing={1} mb={1.5}>
                ...
                />
                {selectedTrip?.facultyResponsible && (
                  <Chip
                    label={selectedTrip.facultyResponsible}
                    variant="outlined"
                    size="small"
                    sx={{ fontWeight: 600, fontSize: "0.75rem" }}
                  />
        ...
        <DialogContent sx={{ p: 3 }}>
          <Stack spacing={2.5}>
              ...
              <Box>
                ...
                <Stack spacing={1.5} sx={{ mt: 1 }}>
                  <Box display="flex" alignItems="center">
                    ...
                    >
                      {selectedTrip.ProfCreator.firstName?.charAt(0)}
                      {selectedTrip.ProfCreator.lastName?.charAt(0)}
                    </Avatar>
                    <Box>
                      <Typography variant="subtitle1" fontWeight={600}>
                        {selectedTrip.ProfCreator.firstName} {selectedTrip.ProfCreator.lastName}
                      </Typography>
            ...
            {selectedTrip?.ProfCreator && <Divider />}
      ...
      <Dialog
        ...
        onClose={() => { setDeleteDialogOpen(false); setSelectedTripToDelete(null); setDeleteError(''); }}
      >
        <DialogTitle>Delete Trip</DialogTitle>
        <DialogContent>
          <Typography>
            {selectedTripToDelete
              ? `Are you sure you want to delete the trip "${selectedTripToDelete.name || 'Unknown'}"?`
        ...
        <DialogActions>
          <Button onClick={() => { setDeleteDialogOpen(false); setSelectedTripToDelete(null); setDeleteError(''); }} disabled={deleteLoading}>Cancel</Button>
          <Button
            onClick={async () => {
              ...
              try {
                ...
                setDeleteError('');
                const result = await deleteTrip(selectedTripToDelete._id);
                if (result.ok) {
                  await fetchEvents(filters);
                  setDeleteDialogOpen(false);
                  setSelectedTripToDelete(null);
                } else {
                  setDeleteError(result.error || 'Failed to delete trip');
                }
              } catch (err: any) {
      ...
      {selectedTrip && (
        ...
        >
          <DialogTitle>
            Register for {selectedTrip?.name}
            <IconButton
