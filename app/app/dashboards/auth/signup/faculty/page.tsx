...
const StaffSignupPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.dummyrole) {
      errors.dummyrole = "Please select a role";
      isValid = false;
    }
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      } else if (errorMessage.toLowerCase().includes("staff id")) {
        setFieldErrors((prev) => ({
          ...prev,
          staffId: errorMessage,
        }));
      } else {
          ...
          <div className="space-y-6">
            ...
            <div>
              <input
                ...
                className={`w-full px-4 py-4 border-b-2 ${
                  fieldErrors.staffId ? "border-red-500" : "border-gray-300"
                } bg-transparent focus:border-gray-900 focus:outline-none transition-colors text-gray-900 placeholder-gray-500`}
              />
              {fieldErrors.staffId && (
                <div className="mt-1 flex items-center text-sm text-red-600">
                  <Info size={16} className="mr-1 flex-shrink-0" />
                  <span>{fieldErrors.staffId}</span>
                </div>
              )}
            </div>
            ...
            <div>
              <select
                name="dummyrole"
                value={formData.dummyrole}
                onChange={handleChange}
                className={`w-full px-4 py-4 border-b-2 ${
                  fieldErrors.dummyrole ? "border-red-500" : "border-gray-300"
                } bg-transparent focus:border-gray-900 focus:outline-none transition-colors text-gray-900 appearance-none cursor-pointer ${
                  !formData.dummyrole ? "text-gray-500" : ""
                }`}
                style={{
              ...
              {fieldErrors.dummyrole && (
                <div className="mt-1 flex items-center text-sm text-red-600">
                  <Info size={16} className="mr-1 flex-shrink-0" />
                  <span>{fieldErrors.dummyrole}</span>
                </div>
              )}
            </div>
